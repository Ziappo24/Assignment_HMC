...
\begin{itemize}
    \item \textbf{Imitation (Dave)} / \textbf{Perception (Simpson)}: The elderly person observes the expert on screen and performs the movement. The system provides instant visual feedback to show macroscopic deviations of the limbs (e.g., arm too low).
...
Posa_1_Difficile-1.mp4,MediaPipe Pose,0,15.32,65.27,0.945,0.998
Posa_1_Difficile-1.mp4,MediaPipe Pose,1,12.87,77.70,0.947,0.999
Posa_1_Difficile-1.mp4,YOLO Pose,0,25.41,39.35,0.920,0.970
Posa_1_Difficile-1.mp4,YOLO Pose,1,20.18,49.55,0.925,0.974
...
Posa_1_Difficile-1.mp4,OpenPose,0,3401.85,0.29,0.887,0.955
Posa_1_Difficile-1.mp4,OpenPose,1,3258.91,0.31,0.890,0.957
\end{lstlisting}
...
The combined analysis of the metrics reveals a clear performance hierarchy. \textbf{MediaPipe Pose} emerges as the most performant model, achieving an average of 75.619 FPS with an inference latency of only 13.324 ms, a mean confidence of 94.7\%, and near-perfect body coverage (99.9\%). These figures position MediaPipe as the optimal choice for real-time feedback applications, comfortably exceeding the 30 FPS threshold required for fluid human-computer interaction. Its low latency ensures that corrective feedback can be delivered with minimal delay, which is essential for supporting the psychomotor learning stages outlined in Bloom's Taxonomy.

\textbf{YOLO Pose} follows closely, with 50.206 FPS and 21.396 ms latency, also surpassing the real-time requirements. While its confidence (92.5\%) and coverage (97.3\%) are slightly lower than MediaPipe's, YOLO remains a highly viable option. The choice between MediaPipe and YOLO ultimately depends on deployment constraints: MediaPipe offers superior performance across all quantitative metrics, whereas YOLO may be preferred in scenarios requiring integration with other YOLO-based detection tasks (e.g., object detection for fall detection or environment monitoring).

Conversely, both \textbf{OpenCV DNN Pose} and \textbf{OpenPose} exhibit prohibitive computational costs that render them unsuitable for domestic deployment. OpenCV DNN Pose achieves only 0.311 FPS with a latency of 3.2 seconds per frame, and a confidence score of just 69.6\%. OpenPose performs similarly poorly (0.301 FPS, 3.3 seconds latency), despite its theoretically robust architecture based on Part Affinity Fields (PAFs). These performance figures make instant feedback impossible, and the high latency would degrade the user experience to an unacceptable level, potentially causing frustration and disengagement in elderly users. Additionally, the low confidence observed in OpenCV DNN Pose (especially on complex poses) would lead to erroneous corrective messages, a scenario that is both clinically and ethically problematic.

Based on these results, \textbf{MediaPipe Pose} is recommended as the primary algorithm for Active Ageing applications. Its superior FPS and confidence metrics ensure that the system can provide reliable, real-time feedback, which is critical for guiding older adults through the correct execution of Tai Chi movements. YOLO Pose remains a valid alternative, particularly in multi-task scenarios where a unified YOLO-based pipeline is already employed.

\section{Source Code and Data Availability}
The complete benchmarking source code, along with the experimental setup, is publicly available on GitHub at the following repository:
...
The proprietary dataset used for the stress-test, consisting of segmented Tai Chi video clips, as well as the pre-trained pose estimation models evaluated in this study, are available for download from the following Google Drive folder:

\url{https://drive.google.com/drive/folders/1y_KmHY4XL35zGt673nxBT7Gg-XtN9qDT?usp=drive_link}
